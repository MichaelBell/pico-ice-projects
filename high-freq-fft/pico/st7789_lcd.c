...
#define PIN_DIN 16
#define PIN_CLK 19
#define PIN_CS 18  // Must be CLK - 1
#define PIN_DC 17
#define PIN_RESET 30
#define PIN_BL 21
...
void st7789_init(ST7789* st, PIO pio, uint sm, uint32_t* data_buf, uint32_t* ctrl_buf)
{
    ...
    //pwm_set_gpio_level(PIN_BL, 30000);
    gpio_put(PIN_BL, 1);
...
bool st7789_is_transfer_complete(ST7789* st)
{
  if (st->transfer_in_progress)
  {
    if (dma_hw->intr & (1u << st->data_chan)) {
      dma_hw->ints0 = 1u << st->data_chan;
      st->transfer_in_progress = false;
    }
  }
  return !st->transfer_in_progress;
}
